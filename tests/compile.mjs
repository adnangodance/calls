...
export async function loadSource(entry) {
  const sources = ['App.tsx', 'PracticePage.tsx', 'practice-client.ts', 'MicrophoneCheck.tsx', 'microphone-check.ts'];
  ...
  try {
    for (const name of sources) {
      ...
      const compiled = ts.transpileModule(source, { compilerOptions: { target: ts.ScriptTarget.ES2022, module: ts.ModuleKind.ESNext, jsx: ts.JsxEmit.ReactJSX } }).outputText
        ...
        .replace(/from ['"]\.\/(PracticePage|practice-client|MicrophoneCheck|microphone-check)['"]/g, (_, name) => `from '${pathToFileURL(paths.get(name)).href}'`);
