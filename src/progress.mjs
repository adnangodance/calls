...
export function canTakeQuiz(progress, duration) {
  return progress?.completed === true || progress?.submitted === true || (duration > 0 && listenedSeconds(progress?.coverage) + 0.01 >= duration * 0.9);
}
...
export function firstUnplayedPosition(coverage = [], duration) {
  let position = 0;
  for (const [start, end] of coverage) {
    if (start > position + 0.08) return position;
    position = Math.max(position, end);
  }
  return position < duration - 0.1 ? position : 0;
}
